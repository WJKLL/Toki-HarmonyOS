...
class _PortWidgetState<T> extends State<PortWidget<T>> {
  ...
  void _handlePanStart(DragStartDetails details) {
    ...
    // POC 埋点:起点是否放行(被拒=端口已占用/校验失败)。
    pocLog(
      'port.start node=${widget.nodeId} port=${widget.port.id} '
      'out=${widget.isOutput} allowed=${result.allowed}',
    );
  ...
  void _handlePanUpdate(DragUpdateDetails details) {
    ...
    // POC 埋点:过程目标命中(命中=应吸附高亮)。
    pocLog(
      'port.move hit=${hitResult == null ? 'none' : '${hitResult.nodeId}:${hitResult.portId}'}',
    );
  ...
  void _handlePanEnd(DragEndDetails details) {
    ...
    final temp = widget.controller.temporaryConnection;
    // POC 埋点:松手时目标状态(complete 或 cancel)。
    pocLog(
      'port.end target=${temp?.targetNodeId == null || temp?.targetPortId == null ? 'null' : '${temp?.targetNodeId}:${temp?.targetPortId}'}',
    );
    if (temp != null &&
      ...
      try {
        final conn = widget.controller.completeConnectionDrag(
          targetNodeId: temp.targetNodeId!,
          targetPortId: temp.targetPortId!,
        );
        pocLog('port.complete ok=${conn != null}');
      } catch (e) {
        pocLog('port.complete ERR $e');
      }
    } else {
            ...
            child: Observer.withBuiltChild(
              builder: (context, child) {
                ...
                return ElementScope(
                  ...
                  // POC(0.2.0):触摸端口按下即拉线(零延迟,不等位移阈值)。
                  instantManualTouch: true,
                  // POC(0.2.1):拖线中第二指 → 平移画布(内容跟手)。
