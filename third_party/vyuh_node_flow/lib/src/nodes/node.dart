...
class Node<T> {
  ...
  /// POC vendor 补丁(v0.5.0 阶段3 hover):鼠标悬停态(仅桌面指针驱动;
  /// 触摸恒 false,零移动端开销)。视觉由自渲染卡按需读取。
  final Observable<bool> _hovered = Observable<bool>(false);
  ...
  /// POC vendor 补丁(v0.5.0 阶段3 hover):鼠标悬停态(controller 驱动;
  /// 触摸恒 false,零移动端开销;视觉由自渲染卡按需读取)。
  bool get isHovered => _hovered.value;

  /// 内部:由 [NodeFlowController.setHoverNode] 维护。
  void setHovered(bool value) => runInAction(() => _hovered.value = value);
