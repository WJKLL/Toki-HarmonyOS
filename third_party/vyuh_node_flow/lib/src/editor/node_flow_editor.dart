...
class _NodeFlowEditorState<T, C> extends State<NodeFlowEditor<T, C>>
              ...
              child: Listener(
                ...
                child: Observer.withBuiltChild(
                  builder: (context, child) {
                    ...
                    // POC vendor 补丁(v0.5.0 阶段3 hover):鼠标移出画布清
                    // hover 态(节点/连线高亮不残留)。
                    return MouseRegion(
                      cursor: cursor,
                      onExit: (PointerExitEvent _) {
                        widget.controller.setHoverNode(null);
                        widget.controller.setHoverConnection(null);
                      },
                      child: child,
                    );
                  },
  ...
  void _handlePointerDown(PointerDownEvent event) {
    ...
    // POC 埋点:确认画布命中判定。
    pocLog(
      'editor.down hit node=${hitResult.isNode} port=${hitResult.isPort} kind=${event.kind}',
    );
