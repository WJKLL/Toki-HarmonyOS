      ...
      children: [
        ...
        Observer(
          builder: (context) {
            ...
            // POC vendor 补丁(v0.5.0 阶段3 hover):悬停连线集合
            // (读 observable 注册依赖 → hover 变化重建快照)。
            final hoveredIds = controller.hoveredConnectionId == null
                ? const <String>{}
                : <String>{controller.hoveredConnectionId!};
            ...
            // POC vendor 补丁(v0.1.7→0.1.10):直接遍历全量连接而非
            // visibleConnections(空间索引裁剪)——新连线入库索引进度依赖
            // 端点脏 flush,存在延迟导致重连不渲染。
            // v0.1.10:不再排除 activeIds —— active 层渲染不稳定(拖动/平移
            // 后连线消失且不恢复)。小图(<百线)静态层每帧重绘代价可接受,
            // active 层在其上叠加同线(同色同路径,视觉无差)。
