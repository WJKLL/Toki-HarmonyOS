...
class ConnectionPainter {
  ...
  ConnectionRenderSnapshot buildRenderSnapshot<T, C>({
    ...
    // POC vendor 补丁(v0.5.0 阶段3 hover):悬停连线集合(渲染高亮,
    // 覆盖选中优先;hoveredConnectionId 非空时由层传入单元素集)。
    Set<String> hoveredIds = const <String>{},
  }) {
    ...
    for (final connection in connections) {
      ...
      // POC vendor 补丁(v0.5.0 阶段3 hover):悬停高亮(选中优先;hover
      // 用主题高亮色,宽度选中-1,视觉区分层级)。
      final isHovered = !isSelected && hoveredIds.contains(connection.id);
      final color = isSelected
          ? connection.selectedColor ?? connectionTheme.selectedColor
          : isHovered
          ? connectionTheme.highlightColor
      ...
      final strokeWidth = isSelected
          ...
          : isHovered
          ? (connection.strokeWidth ?? connectionTheme.strokeWidth) + 1
      ...
      final canBatchOverview =
          simplifyPaths &&
          !isSelected &&
          !isHovered && // POC v0.5.0:hover 线不并入批量,保留单线高亮。
